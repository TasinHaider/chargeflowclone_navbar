...
const IntegrationCard = () => {
    // Ensuring the background gradient matches your global preference
    const cardStyle = { backgroundImage: 'linear-gradient(221deg, #1c1c1c 0%, #151515 98%)' };

    return (
        <section className="w-full bg-transparent">
            {/* Added md: flex and adjusted gap to match other sections exactly */}
            <div className='flex w-full justify-between p-4 bg-transparent gap-2 max-w-[1240px] mx-auto'>

                {/* LEFT LARGE CARD: ALL INTEGRATIONS */}
                <div
                    style={cardStyle}
                    // Explicitly set h-[315px] to match CustomersCard
                    className="relative w-[53%] h-[315px] md:h-[280px] lg:h-[315px] p-8 rounded-2xl overflow-hidden border border-white/5 group cursor-pointer transition-all duration-500 hover:border-white/10"
                >
                ...
                {/* Fixed height of the container to match the left card */}
                <div className='flex flex-col gap-2 flex-1 h-[315px] md:h-[280px] lg:h-[315px]'>

                    {/* STRIPE */}
                    <div
                        style={cardStyle}
                        className="relative flex-1 p-6 rounded-2xl border border-white/5 overflow-hidden group cursor-pointer transition-all duration-500 hover:border-white/10"
                    >
